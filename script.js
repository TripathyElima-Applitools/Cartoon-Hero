...
// Select the menu toggle button (usually a hamburger icon or similar)
const menuToggle = document.querySelector('.menu-toggle');

// Select the <ul> element inside the <nav> that holds the navigation links
const navList = document.querySelector('nav ul');

// Select all anchor links (<a>) inside the <nav> (which are the individual menu items)
const navLinks = document.querySelectorAll('nav ul li a');

// Add a click event listener to the menu toggle (e.g., hamburger icon)
menuToggle.addEventListener('click', () => {
    // Toggle the 'active' class on the navigation list
    // This will typically open or close the mobile menu (for example, showing/hiding the navigation links)
    navList.classList.toggle('active');
});

// Add click event listeners to each navigation link (<a>) to close the menu when clicked
navLinks.forEach(link => {
    link.addEventListener('click', () => {
        // Remove the 'active' class from the navigation list when a link is clicked
        // This effectively closes the menu
